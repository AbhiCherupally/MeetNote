...
class STTService:
    def __init__(self):
        ...
        if not self.api_key:
            ...
            print("✅ AssemblyAI SDK configured")
    ...
    def _format_assemblyai_sdk_response(self, transcript: aai.Transcript) -> List[Dict[str, str]]:
        """Format AssemblyAI SDK transcript to our format"""
        segments = []
        
        # If we have speaker labels (utterances), use them
        if transcript.utterances:
            print(f"📝 Processing {len(transcript.utterances)} utterances with speaker labels")
            ...
            print("📝 No speaker labels, using full transcript")
            if transcript.text:
                segments.append({
                    "timestamp": "0:00",
